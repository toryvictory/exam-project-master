...
export function* activeContestsSaga(action) {
  ...
  try {
    const { data } = yield Api.contestApi.getActiveContests(action.data);
...
export function* customerContestsSaga(action) {
  yield put({ type: ACTION.GET_CONTESTS_ACTION_REQUEST });
  try {
    const { data } = yield Api.contestApi.getCustomersContests(action.data);
    yield put({ type: ACTION.GET_CONTESTS_ACTION_SUCCESS, data });
  } catch (e) {
    yield put({ type: ACTION.GET_CONTESTS_ACTION_ERROR, error: e.response });
  }
...
export function* updateContestSaga(action) {
  ...
  try {
    const { data } = yield Api.contestApi.updateContest(action.data);
...
export function* dataForContestSaga(action) {
  ...
  try {
    const { data } = yield Api.contestApi.dataForContest(action.data);
...
export function* getContestByIdSaga(action) {
  ...
  try {
    const { data } = yield Api.contestApi.getContestById(action.data);
    const { Offers } = data;
    delete data.Offers;
    yield put({
...
export function* downloadContestFileSaga(action) {
  ...
  try {
    const { data } = yield Api.contestApi.downloadContestFile(action.data);
